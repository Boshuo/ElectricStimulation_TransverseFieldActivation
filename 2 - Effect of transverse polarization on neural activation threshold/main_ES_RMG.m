function results = main_ES_RMG(	mod_prmtr, out_ctrl )
%
% mod_prmtr: structure specifying model parameter:
% model_name                Model: 'UF_TrmB', 'UF_TrmS', 'UF_BndO', 'UF_BndA', 'PE_A', 'DE_A'
...
t_main = tic;
[solver, stimulation, cable] = specify_model_ES_RMG( mod_prmtr );	% Specify parameters for solver, stimulation, and cable
...
for ii = 1 : num_sim
    ...
    elseif ~isnan(results.(['th_',CE_str{1}]))                          % If threshold found for default simulation (ii == 1, straigth axon, CE), search parameters adjusted
        solver.thresh_find.amp_init = results.(['th_',CE_str{1}]) * (0.8 + randn(1) * 0.01);	% Initial search amplitude set to 80% of default threshold
