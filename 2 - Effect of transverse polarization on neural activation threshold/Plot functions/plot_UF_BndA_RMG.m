...
ind_alpha = find(ALPHA(:,1)<=(105/180*pi));
...
ind_alpha = find(ALPHA(:,1)>=(105/180*pi));
[C,h]=contour(ALPHA(ind_alpha,ind_PW)/pi*180,log10(PPWW(ind_alpha,ind_PW)),abs(th_CE(ind_alpha,ind_PW))/10,10.^(E_log_cnt_min:(E_log_cnt_max-3)),'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',13,'Interpreter','latex','LabelSpacing',500);
ind_PW =find(PPWW(1,:)<=1);
