function  [exit_flag, t_end] = simulate_SC_HH(solver, stimulation, cable, plot_options)
...
E_Na = cable.E_Na;         % Na reversal potential, in mV
E_K  = cable.E_K;          % K reversal potential, in mV
E_L  = cable.E_L;          % Leakage reversal potential, in mV

g_Na = cable.g_Na;         % Na conductance in mS/cm^2
g_K  = cable.g_K;          % K conductance in mS/cm^2
g_L  = cable.g_L;          % Nodal leak conductance in mS/cm^2

%% Initialize
% Initalize average membrane potential to V_init
phi_m_bar = zeros(N_comp,len_t) + V_init;

% Initialize equivelant conductance and potential to myelin; values of nodes will overwrite during time steps
g_bar = zeros(N_comp,1);
E_bar = zeros(N_comp,1);

if is_MCE
    ER_TP = stimulation.ER_TP;
    TP_weight = cable.TP_weight;
    n_theta = solver.n_theta;    
    Empty_A = zeros(N_comp,n_theta);        % Placeholder for membrane potential and gating particles at node
else
    Empty_A = zeros(N_comp,1);              % Placeholder for membrane potential and gating particles at node
end

phi_m = Empty_A + V_init;
[m,~,h,~,n,~] = HH(phi_m, T, V_rest);
...
if plot_options.if_plot
    ...
    plot(h_axis,[min(t),max(t)],[E_Na,E_Na],'--k');
    plot(h_axis,[min(t),max(t)],[E_K,E_K],'--k');
    plot(h_axis,[min(t),max(t)],[E_L,E_L],'--k');
    plot(h_axis,[min(t),max(t)],[V_rest,V_rest],':k');
    axis tight;ylim(h_axis,[-90,60]);
    xlabel(h_axis,'Time:  $$t \: \mathrm{(ms)}$$','Interpreter','latex');
...
for t_ind = 2:len_t
    ...
    if is_MCE
        g_bar = sum( ( ( g_Na .* m.^3 .* h )  + (g_K .* n.^4) + g_L ) .* TP_weight ,2);       % mS/cm^2
        E_bar = sum( ( ( E_Na - ER_TP * pulse_shape(t_ind) ) .* ( g_Na .* m.^3 .* h ) + ...
                       ( E_K  - ER_TP * pulse_shape(t_ind) ) .* ( g_K  .* n.^4      ) + ...
                       ( E_L                               ) .* ( g_L               ) ) ...
                       .* TP_weight, 2 ) ./ g_bar;     % mV
    else
        g_bar =    ( ( ( g_Na .* m.^3 .* h )  + (g_K .* n.^4) + g_L )                );       % mS/cm^2
        E_bar =    ( ( ( E_Na                              ) .* ( g_Na .* m.^3 .* h ) + ...
                       ( E_K                               ) .* ( g_K  .* n.^4      ) + ...
                       ( E_L                               ) .* ( g_L               ) ) ...
                                       )./ g_bar;      % mV
    end
    ...
    F = tau_mem .* phi_m_bar(:,t_ind-1) + dt(t_ind) * (E_bar + Activating_f);
    A = tau_mem  + dt(t_ind) + dt(t_ind) * Lambda2_diag;
    B = -Lambda2_left  * dt(t_ind);
    C = -Lambda2_right * dt(t_ind);
    
    phi_m_bar(:,t_ind) = tridiag(A, B, C, F);    % Backward Euler step
    
    if any(abs(phi_m_bar(:,t_ind)) > 10e3)
        exit_flag = 0;
        break;
    end
    ...
    if is_MCE
        phi_m = repmat( phi_m_bar(:,t_ind), [1,n_theta] ) + ER_TP * pulse_shape(t_ind);
    else
        phi_m = phi_m_bar(:,t_ind); 
    end
    [m_inf, tau_m, h_inf, tau_h, n_inf, tau_n] = HH(phi_m, T, V_rest);
    % Backward Euler for gating variables
    m = ( m_inf * dt(t_ind) + m .* tau_m) ./ ( dt(t_ind) + tau_m ) ;
    h = ( h_inf * dt(t_ind) + h .* tau_h) ./ ( dt(t_ind) + tau_h ) ;
    n = ( n_inf * dt(t_ind) + n .* tau_n) ./ ( dt(t_ind) + tau_n ) ;
    
    if (phi_m_bar(z_ind_AP,t_ind) > phi_AP ) && (~exit_flag)
        t_end = t(t_ind);
        t_end_ind = t_ind;
        exit_flag=1;
    end
...
if if_plot
    plot(h_axis,t,phi_m_bar(:,:),'b-','LineWidth',1.5);
    plot(h_axis,t(plot_ind_0 + 1),phi_m_bar(:,plot_ind_0 + 1),'cx','LineWidth',2);
    plot(h_axis,t(plot_ind_PW),phi_m_bar(:,plot_ind_PW),'m+','LineWidth',2);
    plot(h_axis,t(t_end_ind),phi_m_bar(:,t_end_ind),'rp','LineWidth',2);
