...
[C,h]=contour(ALPHA(ind,:)/pi*180,log10(PPWW(ind,:)),abs(th_CE(ind,:))/10,10.^(E_log_cnt_min:E_log_cnt_max-1),'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',14,'Interpreter','latex','LabelSpacing',500);
ind = find(ALPHA(:,1)>=(80*pi/180));
contour(ALPHA(ind,:)/pi*180,log10(PPWW(ind,:)),abs(th_CE(ind,:))/10,10.^(E_log_cnt_min:E_log_cnt_max),'-','LineWidth',1.5,'Color',[1,1,1]*0);
