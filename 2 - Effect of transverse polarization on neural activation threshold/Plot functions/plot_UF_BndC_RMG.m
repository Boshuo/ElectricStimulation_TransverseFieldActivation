...
% 
% th_CE_ortho(isnan(th_CE_ortho)) = 1e6;
...
th_per_diff_MCE_antio = compiled_results.th_per_diff_MCE;
% 
% max_noIP = max(th_CE_anti(:));
% th_CE_anti(isnan(th_CE_anti)) = 1e6;
...
[C,h]=contour(alpha_all/pi*180,log10(PW_all),abs(th_CE)/10,10.^(amp_log_min:(amp_log_max-2)),'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',10,'FontWeight','Bold');

ind = find(PW_all(1,:)>=(0.1));
[C,h]=contour(alpha_all(:,ind)/pi*180,log10(PW_all(:,ind)),abs(th_CE(:,ind))/10,10.^([1,1]*(amp_log_max-1)),'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',10,'FontWeight','Bold');
ind = find(PW_all(1,:)<=(0.1));
[C,h]=contour(alpha_all(:,ind)/pi*180,log10(PW_all(:,ind)),abs(th_CE(:,ind))/10,10.^([1,1]*(amp_log_max-1)),'-','LineWidth',1.5,'Color',[1,1,1]*0);
% clabel(C,h,'FontSize',10,'FontWeight','Bold');

% plot([115,108],[-3,1],'w--','LineWidth',1.5);
% plot([45,45],[-3,1],'w--','LineWidth',1.5);
...
for ii = 1:numel(alpha_all)
    ...
    elseif  abs(th_CE_ortho(ii)/th_CE_anti(ii)-1)<1e-2
        ind_dir(ii) = 0;
    elseif abs(th_CE_ortho(ii)) * 0.995 < abs(th_CE_anti(ii))* 1.005
        ind_dir(ii) = 1;
    end
end

ind_dir(( (PW_all > 0.1 & alpha_all > 112.5*pi/180)  | (PW_all > 0.8 & alpha_all>106*pi/180)) & ~ind_dir) = 3;
% ind_dir([336,337])=1;
% ind_dir([335])=0;
...
% ind_dir(ind_0) = 1;
% ind_dir(~ind_0) = 0;
% contour(alpha_all/pi*180,log10(PW_all),ind_dir,[0.5,0.5],'-','LineWidth',2,'Color',[1,1,1]*1);
ind_dir(ind_2) = 1;
ind_dir(~ind_2) = 0;
contour(alpha_all/pi*180,log10(PW_all),ind_dir,[0.5,0.5],'-','LineWidth',2,'Color',[1,1,1]*1);
ind_dir(ind_1) = 1;
...
colormap(cm1);
caxis([amp_log_min,amp_log_max]);
% surf(log10(H_all/1e-1),YY_all/1e-1,H_all*0,log10(abs(th_MCE)/10),'LineStyle','-');
contourf(alpha_all/pi*180,log10(PW_all),log10(abs(th_MCE)/10),amp_log_min:0.01:amp_log_max,'LineStyle','none');

[C,h]=contour(alpha_all/pi*180,log10(PW_all),abs(th_MCE)/10,10.^round(amp_log_min:amp_log_max-2),'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',10,'FontWeight','Bold');

ind = find(PW_all(1,:)>=(0.02));
[C,h]=contour(alpha_all(:,ind)/pi*180,log10(PW_all(:,ind)),abs(th_MCE(:,ind))/10,10.^([1,1]*(amp_log_max-1)),'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',10,'FontWeight','Bold');
ind = find(PW_all(1,:)<=(0.02));
[C,h]=contour(alpha_all(:,ind)/pi*180,log10(PW_all(:,ind)),abs(th_MCE(:,ind))/10,10.^([1,1]*(amp_log_max-1)),'-','LineWidth',1.5,'Color',[1,1,1]*0);
% clabel(C,h,'FontSize',10,'FontWeight','Bold');
...
set(hcb,'TickDirection','out');
set(hcb,'TickLabelInterpreter','latex');
set(hcb,'FontSize',14);set(hcb,'Ticks',ticks);
set(hcb,'TickLabels',ticklabels);
...
% ind_dir([109,177,318:322,337:345,360:345,360:368,383:390,406:410]) = 2;
...
[C,h]=contour(alpha_all/pi*180,log10(PW_all),th_per_diff_MCE,[-2.5,-2.5],'r--','LineWidth',2);

ind = find(PW_all(1,:)>=(0.11));
ind_alpha= find(alpha_all(:,1)<=115*pi/180);
[C,h]=contour(alpha_all(ind_alpha,ind)/pi*180,log10(PW_all(ind_alpha,ind)),th_per_diff_MCE(ind_alpha,ind),[-25,-50],'-','LineWidth',1.5,'Color',[1,1,1]*0);
ind = find(PW_all(1,:)>=(0.1));
ind_alpha= find(alpha_all(:,1)>=115*pi/180);
[C,h]=contour(alpha_all(ind_alpha,ind)/pi*180,log10(PW_all(ind_alpha,ind)),th_per_diff_MCE(ind_alpha,ind),[-25,-50],'-','LineWidth',1.5,'Color',[1,1,1]*0);
clabel(C,h,'FontSize',10,'FontWeight','Bold');
...
% plot([115,111],[-3,1],'w--','LineWidth',2);
% plot([115,112],[-3,0],'w--','LineWidth',2);
